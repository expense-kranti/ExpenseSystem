...
public class SessionManager {
	...
	/**
	 * This is an annonympus session which never expires and is not stored on
	 * cache or DB
	 */
	 ...
	 */
	private static int sessionTimeOut = 43200 * 60 * 1000;
	...
	/**
	 * This method gets a session with the given id. The method first checks if
	 * the session is on cache, if not then the method gets it from database
	 * 
	 * @param sessionId
	 *            The id of the session
	 * @return A session
	 * @throws ValidationFailedException
	 *             This exception will not be thrown as it is required by
	 *             validation failed exception which in this case is made to
	 *             false
	 */
	...
	public void saveSessionOnExit(Session session) {
		...
		try {
			queueReaderJob.requestBackroundWorkItem(session, subjects, "SessionManager", "getSession");
		} catch (Exception ex) {
	...
	private Session getSessionFromCache(String sessionId) {
		...
		try {
			...
			logger.logException("SessionManager", "getSession", "Get Session From cache", ex.toString(), ex);
		}
		return session;
	}
	...
	private void putSessionOnCache(Session session) {
		try {
			if (CacheFactory.getInstance().isCacheEnabled()) {
				CacheFactory.getInstance().add(Constants.SESSION + session.getSessionId(), session, sessionTimeOut);
			}
		} catch (Exception ex) {
	...
	/**
	 * This method creates a session for the given user and saves it to the
	 * database and cache. If the user is not saved to cache it will not throw
	 * an exception however if there is an issue saving the session to DB there
	 * will be an error
	 * 
	 * @param externalFacingUser
	 *            This is the user for whom session is created
	...
	public void initialize() {
		sessionTimeOut = Integer.parseInt(configurationManager.get("SessionTimeOutInMinutes") == null
				? configurationManager.get("SessionTimeOutInMinutes")
				: configurationManager.get("SessionTimeOutInMinutes")) * 60;
	...
	/**
	 * This method cleansup old and expired sessions. It is a good practice to
	 * cleanup sessions from code rather than to rely upon a DBA. Further the
	 * logs we generate keep all the information about the session and user to
	 * trace any actitvity.
	 */
	...
	private Session getAnnonymousSession() {
		if (this.annonymousSession == null) {
			try {
				// ExternalFacingUser annonymousUser =
				// userService.get("ANNONYMOUS");
				// this.annonymousSession = new Session(annonymousUser);
			} catch (Exception ex) {
				// this exception is not expected as the said user is always
				// found
			}
	...
	public Session getBackgroundJobSession() {
		...
		if (this.backroundJobSession == null) {
			try {
				// ExternalFacingUser backGroundJObUser =
				// userService.get("AKS:BACKGROUND");
	...
	public void logout(String sessionId) {
		try {
			if (CacheFactory.getInstance().isCacheEnabled()) {
				CacheFactory.getInstance().remove(Constants.SESSION + sessionId.toUpperCase());
			}
		} catch (Exception e) {
