...
public class CleanupCRMPublishHash {
	 ...
	 * 
	 * @param IAuthTokenService
	 *            The authtoken service
	 */
	...
	public void cleanupSFUpdateHash() throws Exception {
		// if publish queue flag is true only then clean up SFUpdateHash and
		...
		if (Boolean.parseBoolean(configurationManager.get("IsPublishQueueEnabled"))) {
			...
			try {
				Set<String> hashKeysSet = redisSFUpdateHashAccess
						.keys(configurationManager.get("SF_Update_Hash_Name") + ":AKS:??????????");
				...
				logger.logException("CleanupCRMPublishHash", "PublishBulkHashData",
						"inside try block" + exception.getMessage(), "during traversing of hashmap exception comes",
						exception);
			}
		}
	}
	...
	private void publishBulkHashData(String jsonBody) throws IOException, UnauthorizedException, NotFoundException {
		...
		logger.logInfo("CleanupCRMPublishHash", "PublishBulkHashData", "", "Publishing user update data" + jsonBody
				+ "to url" + configurationManager.get("SF_Update_Account_Publish_URL"));
		HttpResponse httpResponse = HttpUtility.makeHttpRequest(
				configurationManager.get("SF_Update_Account_Publish_URL"), requestHeaders, null, jsonBody,
				configurationManager.get("SF_Update_Account_Publish_Method"));
		String responseBody = httpResponse.getResponseBody();
		if (responseBody == null) {
			responseBody = "publish response is null";
		}
		logger.logInfo("CleanupCRMPublishHash", "PublishBulkHashData", "",
				"Response status:" + httpResponse.getHttpStatus() + "Publish Response Body:" + responseBody);
		...
		if (httpResponse.getHttpStatus() == 200) {
			logger.logInfo("CleanupCRMPublishHash", "PublishBulkHashData", "publishResponse",
					httpResponse.getResponseBody());
		} else {
			logger.logError("CleanupCRMPublishHash", "PublishBulkHashData", "publishResponse",
	...
	private void sendExceptionEmail(HttpResponse httpResponse) {
		...
		try {
			...
			// configurationManager.get("SF_Update_Account_Publish_Subject"),
			// httpResponse.getRequestBody(), httpResponse.getResponseBody());
		} catch (Exception e) {
			logger.logInfo("CleanupCRMPublishHash",
					"PublishBulkHashData" + configurationManager.get("Publish_Bulk_HashData"), "publishResponse",
					httpResponse.getResponseBody());
		}
	}
	...
	private void rePublish(String jsonBody) throws IOException, NotFoundException {
		...
		if (httpResponse.getHttpStatus() == 200) {
			...
			logger.logError("CleanupCRMPublishHash", "PublishBulkHashData", "rePublishResponse",
					httpResponse.getResponseBody());
			sendExceptionEmail(httpResponse);
		}
	}
