...
public class UserService implements IUserService {
	...
	// getReportInputEntity(ExternalFacingReturnedUser user) {
	// // check state of user if user state is experian attempt or
	// // authquestion then give its report input entity
	//
	// List<ReportInputEntity> reportInputEntityList =
	// mysqlReport.getReportInputEntity(user.getUserId());
	...
	public Session authenticateByOTP(AuthenticationRequest authenitcationRequest) throws UnauthorizedException {
		...
		try {
			...
			// Commented as not being deployed to prod
			// user = getReportInputEntity(user);
