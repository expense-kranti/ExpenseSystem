	...
	public void publishUserAndAssessmentReport() throws UnauthorizedException, NotFoundException, BadRequestException {
		...
		try{
			queueReaderJob.requestBackroundWorkItem(
					"", subjectsForPublishUserReportData, "ScriptService", "publishUserAndAssessmentReport");
		}	
		catch(Exception ex){
			logger.logError("ScriptsService", "publishUserAndAssessmentReport", "Inside try-catch block", ex.toString());
		}	
	}
	
	/**
     * This method checks the roles of the user and tells about whether that user have managerial role or not
