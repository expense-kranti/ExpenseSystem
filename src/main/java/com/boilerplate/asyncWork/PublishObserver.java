...
public class PublishObserver extends BaseRedisDataAccessLayer implements IAsyncWorkObserver {
	 ...
	 */
	public void setRedisSFUpdateHashAccess(RedisSFUpdateHash redisSFUpdateHashAccess) {
	...
	private void pushAgainQueue(PublishEntity publishEntity) {
		...
		try {
			queueReaderJob.requestBackroundWorkItem(publishEntity, subjects, "PublishLibrary",
					"requestPublishAsyncOffline");
		} catch (Exception ex) {
	...
	public void publish(PublishEntity publishEntity)
			throws IOException, NotFoundException, UnauthorizedException, Exception {
		// PublishData instantiation
		switch (publishEntity.getPublishSubject()) {
		case "UPDATE_LOGGED_IN_USER_AKS":
			ExternalFacingReturnedUser externalFacingReturnedUser = Base
					.fromJSON(publishEntity.getReturnValue().toString(), ExternalFacingReturnedUser.class);
			redisSFUpdateHashAccess.hmset(
					configurationManager.get("SF_Update_Hash_Name") + ":" + externalFacingReturnedUser.getId(),
					createSFHashMap(publishEntity, externalFacingReturnedUser));
	 ...
	 * @param publishEntity
	 *            The publishEntity
	 * @param externalFacingReturnedUser
	 *            The externalFacingReturnedUser
	 ...
	 *             The IOException
	 */
	public Map<String, String> createSFHashMap(PublishEntity publishEntity,
		...
		sfUpdateHashEntity.setAccountId(
				externalFacingReturnedUser.getCrmid() == null ? "" : externalFacingReturnedUser.getCrmid());
		sfUpdateHashEntity.setAlternateNumber(externalFacingReturnedUser.getAlternateNumber() == null ? ""
				: externalFacingReturnedUser.getAlternateNumber());
		sfUpdateHashEntity.setLocation(
				externalFacingReturnedUser.getLocation() == null ? "" : externalFacingReturnedUser.getLocation());
		sfUpdateHashEntity.setEmploymentStatus(externalFacingReturnedUser.getEmploymentStatus() == null ? ""
				: externalFacingReturnedUser.getEmploymentStatus().toString());
		sfUpdateHashEntity.setDob(
				externalFacingReturnedUser.getDateOfBirth() == null ? "" : externalFacingReturnedUser.getDateOfBirth());
		sfUpdateHashEntity.setReferUUID(
				externalFacingReturnedUser.getUserReferId() == null ? "" : externalFacingReturnedUser.getUserReferId());
		sfUpdateHashEntity.setFeedbackStatus(externalFacingReturnedUser.getIsFeedBackSubmitted() == false ? ""
				: String.valueOf(externalFacingReturnedUser.getIsFeedBackSubmitted()));
		sfUpdateHashEntity.setTotalScore(
				externalFacingReturnedUser.getTotalScore() == null ? "" : externalFacingReturnedUser.getTotalScore());
		Map<String, String> sfUpdateHashMap = new ObjectMapper().readValue(sfUpdateHashEntity.toJSON(),
	...
	private void requestBackroundWorkItem(PublishEntity publishEntity, String subject) throws Exception {
		...
		try {
			this.queueReaderJob.requestBackroundWorkItem(publishEntity, subjects, "PublishObserver",
					"requestBackroundWorkItem", subject + "_");
		} catch (Exception ex) {
			try {
				...
				sendEmailOnUnsuccessfulBulkPublish.sendEmail(tosEmailList, ccsEmailList, bccsEmailList,
						publishEntity.getPublishSubject() + "Push in Bulk Publish queue failure",
						publishEntity.getReturnValue().toString(), "Exception :" + ex);
			} catch (Exception exe) {
				logger.logException("PublishObserver", "requestBackroundWorkItem",
						"Exception Block - Sending Email failure while Pushing in Bulk Publish queue failure",
						ex.toString(), exe);
			}
		}
	}
