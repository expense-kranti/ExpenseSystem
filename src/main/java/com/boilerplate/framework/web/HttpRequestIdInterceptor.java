 ...
 * debugging and client escalation / logging process. The said id is sent back
 * to users on the header X-Http-Request-Id The class also sets things like
 * session etc on the thread
 */
@Component
public class HttpRequestIdInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// if the request for authentication then session doesnt make sense so
		...
		if (this.isNonSessionURL(request) == false) {
			...
			// if the session id is not null get the session from the
			// cache/database
	...
	/**
	 * This method sets the paramteres of session in the response, including
	 * Header and cookie
	 * 
	 * @param reponse
	 *            The http response
	...
	/**
	 * This method returns true if the request is an authentication request and
	 * false if it is not
	 * 
	 * @param request
	 *            The Http request
	...
	/**
	 * This method finds the session token if it exists in cookie, header or
	 * query string.
	 * 
	 * @param request
