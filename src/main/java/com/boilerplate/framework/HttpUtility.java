...
public class HttpUtility {
	 ...
	 *            The cookies to be sent
	 * @param requestBody
	 *            The request body
	 * @param responseHeaders
	 *            The response headers
	 * @param responseCookies
	 *            The response cookie's
	 * @param responseBody
	 *            The response body
	 * @param method
	 *            The http method
	 ...
	 * @throws IOException
	 *             If there is an error in creating the http request.
	 */
	public static HttpResponse makeHttpRequest(String url,
		...
		try {
			...
			logger.logError("HttpUtility", "makeHttpRequest", "Catch Exception Block - setConnectTimeout ",
					"URL :" + url + "& Request Header:" + Base.toJSON(requestHeaders) + "& Request Cookies:"
							+ Base.toJSON(requestCookies) + "& Request Body:" + requestBody + "& Method:" + method
							+ "& Exception:" + exception);
		}
		...
		if (!method.equals("GET") && requestBody != null) {
			try {
				...
				// Log Error
				logger.logError("HttpUtility", "makeHttpRequest", "Catch Exception Block 1",
						"URL :" + url + "& Request Header:" + Base.toJSON(requestHeaders) + "& Request Cookies:"
								+ Base.toJSON(requestCookies) + "& Request Body:" + requestBody + "& Method:" + method
								+ "& Exception:" + ex);
				throw ex;
		...
		if (responseCode == 200) {
			bufferdReader = new BufferedReader(new InputStreamReader(httpURLConnection.getInputStream()));
		} else {
			bufferdReader = new BufferedReader(new InputStreamReader(httpURLConnection.getErrorStream()));
		}
		String inputLine;
		try {
			...
			// Log Error
			logger.logError("HttpUtility", "makeHttpRequest", "Catch Exception Block 2",
					"URL :" + url + "& Request Header:" + Base.toJSON(requestHeaders) + "& Request Cookies:"
							+ Base.toJSON(requestCookies) + "& Request Body:" + requestBody + "& Method:" + method
							+ "& Exception:" + ex);
			throw ex;
